    ...
    def run(self, initial_population, num_steps, energy_function, min_pos, max_pos, oracle, log_every=50):
        ...
        with torch.no_grad():
            ...
            for i in range(num_steps):
                ...
                nmut = mut_distance(x_proposal, energy_function.wt_onehot)
                proposal_mask = nmut > self.nmut_threshold
                proposal_energy[proposal_mask] = -np.inf
                proposal_fitness[proposal_mask] = -np.inf
                ...
                state_seqs = torch.chunk(x_new.clone(), n_chains)
